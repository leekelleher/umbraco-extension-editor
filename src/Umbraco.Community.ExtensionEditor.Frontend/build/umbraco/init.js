...
export default function initUmbraco(args) {
    ...
    const solutionFilePath = join(rootDir, 'Umbraco.Community.ExtensionEditor.sln');
    ...
    try {
        ...
        execSync(`dotnet add "${join(projectDir, projectName + '.csproj')}" reference "${join(rootDir, 'Umbraco.Community.ExtensionEditor.Backend', 'Umbraco.Community.ExtensionEditor.Backend.csproj')}"`, { stdio: 'inherit' });
